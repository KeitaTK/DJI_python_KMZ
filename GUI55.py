...
"""
convert_height_gui_asl.py (ver. GUI57 – ジンバルヨー自動維持対応版)

修正点
• 機体ヨー角「元を維持」時にジンバルヨーも自動維持
• ジンバルピッチ制御直後にジンバルヨー制御を追加
• ズームカメラの倍率設定機能
• 動画撮影時の全ウェイポイントでのジンバル制御
"""
...
# --- 修正：完全なジンバル角度・ズーム情報取得 --------------------------------
def extract_original_gimbal_angles(tree):
    """orientedShootアクションからジンバル情報とズーム情報を完全取得"""
    original = {}
        ...
        for action in pm.findall(".//wpml:action", NS):
            ...
            if func_elem is not None and func_elem.text == "orientedShoot":
                ...
                if param is not None:
                    ...
                    focal_elem = param.find("wpml:focalLength", NS)
                    
                    gimbal_info = {}
                    if pitch_elem is not None:
                        gimbal_info["pitch"] = float(pitch_elem.text)
                    if yaw_elem is not None:
                        gimbal_info["yaw"] = float(yaw_elem.text)
                    if head_elem is not None:
                        gimbal_info["heading"] = float(head_elem.text)
                    if focal_elem is not None:
                        focal_length = float(focal_elem.text)
                        zoom_ratio = focal_length / 24.0  # 24mmを基準とした倍率計算
                        gimbal_info["focal_length"] = focal_length
                        gimbal_info["zoom_ratio"] = zoom_ratio
                    
                    if gimbal_info:  # 何らかの情報があれば保存
                        original[idx] = gimbal_info
                break
    return original

def zoom_ratio_to_focal_length(zoom_ratio):
    """ズーム倍率から焦点距離に変換"""
    return zoom_ratio * 24.0
...
# --- 新機能：ジンバルヨー制御アクション作成 ----------------------------------
def create_gimbal_yaw_action(ag, gimbal_yaw_angle):
    """ジンバルヨー制御専用のアクションを作成"""
    gyaw_action = etree.SubElement(ag, f"{{{NS['wpml']}}}action")
    etree.SubElement(gyaw_action, f"{{{NS['wpml']}}}actionId").text = "0"
    etree.SubElement(gyaw_action, f"{{{NS['wpml']}}}actionActuatorFunc").text = "gimbalRotate"
    gyaw_param = etree.SubElement(gyaw_action, f"{{{NS['wpml']}}}actionActuatorFuncParam")
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalRotateMode").text = "absoluteAngle"
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalPitchRotateEnable").text = "0"
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalPitchRotateAngle").text = "0"
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalRollRotateEnable").text = "0"
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalRollRotateAngle").text = "0"
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalYawRotateEnable").text = "1"
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalYawRotateAngle").text = str(gimbal_yaw_angle)
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalRotateTimeEnable").text = "0"
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}gimbalRotateTime").text = "0"
    etree.SubElement(gyaw_param, f"{{{NS['wpml']}}}payloadPositionIndex").text = "0"
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
    ...
    if do_photo and not do_video:
        ...
        for pm in placemarks:
            ...
            # NEW: ジンバルヨー制御（機体ヨー「元を維持」時のみ）
            if yaw_fix and yaw_mode == "original" and original_angles and idx in original_angles:
                gimbal_yaw_target = original_angles[idx].get("yaw")
                if gimbal_yaw_target is not None:
                    create_gimbal_yaw_action(ag, gimbal_yaw_target)

            # zoomAction - ズーム設定
            focal_target = None
            if do_zoom and "Zoom" in sensor_modes:
                if zoom_mode == "original" and original_angles and idx in original_angles:
                    focal_target = original_angles[idx].get("focal_length")
                elif zoom_ratio is not None:
                    focal_target = zoom_ratio_to_focal_length(zoom_ratio)
                    
            if focal_target is not None:
                zoom_action = etree.SubElement(ag, f"{{{NS['wpml']}}}action")
                etree.SubElement(zoom_action, f"{{{NS['wpml']}}}actionId").text = "0"
                etree.SubElement(zoom_action, f"{{{NS['wpml']}}}actionActuatorFunc").text = "zoom"
                zoom_param = etree.SubElement(zoom_action, f"{{{NS['wpml']}}}actionActuatorFuncParam")
                etree.SubElement(zoom_param, f"{{{NS['wpml']}}}focalLength").text = str(focal_target)
                etree.SubElement(zoom_param, f"{{{NS['wpml']}}}payloadPositionIndex").text = "0"
    ...
    # 動画モード（修正：全ウェイポイントでジンバル・ヨー制御＋ジンバルヨー追加）
    if do_video:
        pms = sorted(
            [p for p in tree.findall(".//kml:Placemark", NS)
             if p.find("wpml:index", NS) is not None],
        ...
        if pms:
            ...
            # NEW: 最初のウェイポイントでのジンバルヨー制御（機体ヨー「元を維持」時のみ）
            if yaw_fix and yaw_mode == "original" and original_angles and first_idx in original_angles:
                gimbal_yaw_target = original_angles[first_idx].get("yaw")
                if gimbal_yaw_target is not None:
                    create_gimbal_yaw_action(ag, gimbal_yaw_target)
            ...
            focal_target = None
            if do_zoom and "Zoom" in sensor_modes:
                if zoom_mode == "original" and original_angles and first_idx in original_angles:
                    focal_target = original_angles[first_idx].get("focal_length")
                elif zoom_ratio is not None:
                    focal_target = zoom_ratio_to_focal_length(zoom_ratio)
            ...
            # 中間ウェイポイントでのジンバル・ヨー制御
            for pm in pms[1:-1]:  # 最初と最後を除く中間ウェイポイント
                idx = int(pm.find("wpml:index", NS).text)
                
                # ジンバル・ヨー制御が必要な場合のみアクショングループを作成
                need_action = False
                actions_to_add = []
                ...
                if yaw_target is not None:
                    need_action = True
                    actions_to_add.append(("rotateYaw", {
                        "aircraftHeading": str(int(yaw_target)),
                ...
                if pitch_target is not None:
                    need_action = True
                    actions_to_add.append(("gimbalRotate", {
                        "gimbalRotateMode": "absoluteAngle",
                        "gimbalPitchRotateEnable": "1",
                        "gimbalPitchRotateAngle": str(int(pitch_target)),
                ...
                if yaw_fix and yaw_mode == "original" and original_angles and idx in original_angles:
                    ...
                    if gimbal_yaw_target is not None:
                        need_action = True
                        actions_to_add.append(("gimbalRotateYaw", {
                            "gimbalRotateMode": "absoluteAngle",
                            ...
                            "gimbalYawRotateAngle": str(gimbal_yaw_target),
                            "gimbalRotateTimeEnable": "0",
                            "gimbalRotateTime": "0",
                            "payloadPositionIndex": "0"
                        }))
                ...
                if focal_target is not None:
                    need_action = True
                    actions_to_add.append(("zoom", {
                        "focalLength": str(focal_target),
                        "payloadPositionIndex": "0"
                    }))
                
                if need_action:
                    ag_mid = etree.SubElement(pm, f"{{{NS['wpml']}}}actionGroup")
                    etree.SubElement(ag_mid, f"{{{NS['wpml']}}}actionGroupId").text = str(idx)
                    etree.SubElement(ag_mid, f"{{{NS['wpml']}}}actionGroupStartIndex").text = str(idx)
                    etree.SubElement(ag_mid, f"{{{NS['wpml']}}}actionGroupEndIndex").text = str(idx)
                    etree.SubElement(ag_mid, f"{{{NS['wpml']}}}actionGroupMode").text = "sequence"
                    trg_mid = etree.SubElement(ag_mid, f"{{{NS['wpml']}}}actionTrigger")
                    etree.SubElement(trg_mid, f"{{{NS['wpml']}}}actionTriggerType").text = "reachPoint"
                    
                    for action_func, params in actions_to_add:
                        action = etree.SubElement(ag_mid, f"{{{NS['wpml']}}}action")
                        etree.SubElement(action, f"{{{NS['wpml']}}}actionId").text = "0"
                        if action_func == "gimbalRotateYaw":
                            etree.SubElement(action, f"{{{NS['wpml']}}}actionActuatorFunc").text = "gimbalRotate"
                        else:
                            etree.SubElement(action, f"{{{NS['wpml']}}}actionActuatorFunc").text = action_func
                        param_elem = etree.SubElement(action, f"{{{NS['wpml']}}}actionActuatorFuncParam")
                        for key, value in params.items():
                            etree.SubElement(param_elem, f"{{{NS['wpml']}}}{key}").text = value

            # stopRecord at last waypoint
    ...
    try:
        ...
        for kml in kmls:
            ...
            for pm in wpms:
                ...
                # ジンバル・ズーム情報表示
                gimbal_info = original_angles.get(idx, {})
                pitch = gimbal_info.get("pitch", "N/A")
                yaw = gimbal_info.get("yaw", "N/A")
                heading = gimbal_info.get("heading", "N/A")
                focal_length = gimbal_info.get("focal_length", "N/A")
                zoom_ratio_orig = gimbal_info.get("zoom_ratio", "N/A")
                
                if zoom_ratio_orig != "N/A":
                    zoom_info = f"{zoom_ratio_orig:.1f}倍({focal_length:.1f}mm)"
                else:
                    zoom_info = "N/A"
                
                log.insert(tk.END, 
                    f"[WP {idx}] 標高モード={height_mode}, 高度={height_val}, "
                    f"ジンバルピッチ={pitch}°, ジンバルヨー={yaw}°, 機体ヘディング={heading}°, "
                    f"ズーム={zoom_info}\n"
                )
