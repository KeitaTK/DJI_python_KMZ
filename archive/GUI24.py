...
"""
convert_height_gui_asl.py (ver. GUI23)
...
・写真／動画排他制御 (★動画選択時は写真アクションを完全削除)
・撮影オフ時のみジンバル制御可能
・撮影オン時はジンバル制御固定
・ヨー角固定オプション
・撮影モード選択：ワイド／ズーム／IR センサー
・出力KMZに元の.wpmlファイルを含めない
"""
...
def convert_kml(tree: etree._ElementTree,
    ...
    def _get_or_create_action_group(placemark, wp_index, current_max_group_id):
        ...
        for elem in action_group.findall('.//wpml:actionId', NS):
            if elem.text and elem.text.isdigit():
                max_action_id = max(max_action_id, int(elem.text))
    ...
    for pm in tree.findall(".//kml:Placemark", NS):
        for tag, val in [("waypointSpeed",    str(speed)),
                         ("useGlobalSpeed",   "0")]:
    ...
    for ag in tree.findall(".//wpml:actionGroup", NS):
        for act in list(ag.findall("wpml:action", NS)):
            ...
            if f is None or f.text is None:
                continue
            if f.text in ("orientedShoot", "startRecord", "stopRecord",
    ...
    # 4-2. 写真撮影 (orientedShoot) - ★動画選択時は追加しない
    if do_photo and not do_video:
        placemarks = [p for p in tree.findall(".//kml:Placemark", NS)
    ...
    if do_video:
        ...
        if all_placemarks:
            ...
            group_start, next_action_id_start = _get_or_create_action_group(
                first_wp, first_wp.find("wpml:index", NS).text, max_group_id)
            ...
            group_stop, next_action_id_stop = _get_or_create_action_group(
                last_wp, last_wp.find("wpml:index", NS).text, max_group_id + 1)
    ...
    if yaw_fix and yaw_angle is not None:
        for heading_param in (tree.findall(".//wpml:waypointHeadingParam", NS) +
...
class AppGUI(ttk.Frame):
    def __init__(self, master):
        ...
        ttk.Label(self, text="基準高度:").grid(row=0, column=0,
                                              sticky="w")
        ...
        ttk.Label(self, text="速度 (1–15 m/s):").grid(
            row=1, column=0, sticky="w", pady=5)
        self.sp = tk.IntVar(value=15)
        ttk.Spinbox(self, from_=1, to=15, textvariable=self.sp,
                    width=5).grid(row=1, column=1, sticky="w",
                                 columnspan=2)

        self.ph = tk.BooleanVar(value=False)
        ttk.Checkbutton(self, text="写真撮影", variable=self.ph,
                        command=self.update_ctrl).grid(
            row=2, column=0, sticky="w")
        self.vd = tk.BooleanVar(value=False)
        ttk.Checkbutton(self, text="動画撮影", variable=self.vd,
                        command=self.update_ctrl).grid(
            row=2, column=1, sticky="w")
        self.vd_suffix_label = ttk.Label(self, text="動画ファイル名:")
        self.vd_suffix_var   = tk.StringVar(value="video_01")
        self.vd_suffix_entry = ttk.Entry(self, textvariable=self.vd_suffix_var,
                                          width=20)

        ttk.Label(self, text="センサー選択:").grid(
            row=3, column=0, sticky="w")
        self.sm_vars = {mode: tk.BooleanVar(value=False)
                        for mode in SENSOR_MODES}
        for i, mode in enumerate(SENSOR_MODES):
            ttk.Checkbutton(self, text=mode,
                            variable=self.sm_vars[mode]).grid(
                row=3, column=1+i, sticky="w")

        self.gm = tk.BooleanVar(value=True)
        self.gc = ttk.Checkbutton(self, text="ジンバル制御",
                                  variable=self.gm)
        self.gc.grid(row=4, column=0, sticky="w", pady=5)

        self.yf = tk.BooleanVar(value=False)
        ttk.Checkbutton(self, text="ヨー固定", variable=self.yf,
                        command=self.update_yaw).grid(
            row=5, column=0, sticky="w")
        self.yc = ttk.Combobox(self, values=list(YAW_OPTIONS),
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI23)")
    ...
    drop_label = tk.Label(frm, text=".kmzをここにドロップ",
                          bg="lightgray", width=70, height=5,
                          relief=tk.RIDGE)
