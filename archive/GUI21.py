...
"""
convert_height_gui_asl.py (ver. GUI20-Final)
— 機能 —
・高度オフセット＋EGM96モード変換
・速度設定（1–15 m/s）
・写真撮影オプション
...
class AppGUI(ttk.Frame):
    ...
    def update_ctrl(self):
        ...
        if self.ph.get() or self.vd.get(): self.gc.state(['disabled']); self.gm.set(True)
        else: self.gc.state(['!disabled'])
        if self.vd.get():
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI20-Final)")
