...
"""
convert_height_gui_asl.py (ver. GUI54 – 拡張ログ出力版)

変更点
• リアルタイムログに以下を出力:
  - 各ポイントの標高モード・高度・ジンバルピッチ
  - 総ウェイポイント数
  - 使用センサーモード
• 写真撮影と動画撮影のチェックボックス排他制御
"""
            ...
            if func is not None and func.text == "orientedShoot":
                ...
                if param is not None:
                    ...
                    if pitch_elem is not None:
                        original[idx] = float(pitch_elem.text)
                break
    return original
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
        ...
        for pm in placemarks:
            ...
            if yaw_fix:
                if yaw_mode == "original" and original_angles and idx in original_angles:
                    yaw_target = original_angles[idx]
            ...
            if do_gimbal:
                if gimbal_pitch_mode == "original" and original_angles and idx in original_angles:
                    pitch_target = original_angles[idx]
    ...
    if do_video:
        ...
        if pms:
            ...
            # rotateYaw & gimbalRotate before recording
            yaw_target = None
            if yaw_fix:
                if yaw_mode == "original" and original_angles and first_idx in original_angles:
                    yaw_target = original_angles[first_idx]
                elif yaw_angle is not None:
                    yaw_target = yaw_angle
            if yaw_target is not None:
                yaw_action = etree.SubElement(ag, f"{{{NS['wpml']}}}action")
            ...
            if do_gimbal:
                if gimbal_pitch_mode == "original" and original_angles and first_idx in original_angles:
                    pitch_target = original_angles[first_idx]
                elif gimbal_pitch_angle is not None:
                    pitch_target = gimbal_pitch_angle
            if pitch_target is not None:
                gp_action = etree.SubElement(ag, f"{{{NS['wpml']}}}action")
...
def process_kmz(path, offset, do_photo, do_video, video_suffix,
    ...
    try:
        ...
        # センサーモード
        log.insert(tk.END, f"センサーモード: {', '.join(sensor_modes) or 'None'}\n")
        ...
        for kml in kmls:
            ...
            original_pitches = extract_original_gimbal_angles(tree)
            for pm in wpms:
                idx = int(pm.find("wpml:index", NS).text)
                mode = pm.find("wpml:heightMode", NS).text if pm.find("wpml:heightMode", NS) is not None else "None"
                h_elem = pm.find("wpml:ellipsoidHeight", NS) or pm.find("wpml:height", NS)
                height_val = h_elem.text if h_elem is not None else "None"
                pitch_val = original_pitches.get(idx, "N/A")
                log.insert(tk.END, f"[WP {idx}] 標高モード={mode}, 高度={height_val}, 元ジンバルピッチ={pitch_val}\n")

            # 変換と書き出し
            convert_kml(tree, offset, do_photo, do_video, video_suffix,
                        do_gimbal, gimbal_pitch_angle, gimbal_pitch_mode,
                        yaw_fix, yaw_angle, yaw_mode, speed, sensor_modes, hover_time,
                        coordinate_deviation, original_pitches)
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI54)")
