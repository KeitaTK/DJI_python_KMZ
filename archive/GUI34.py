...
# -*- coding: utf-8 -*-
"""
convert_height_gui_asl.py (ver. GUI30 - ヨー角制御確実版)
変更点
・ヨー角固定チェック時に確実に動作するよう、GUI30の２の方式を実装
・その他のGUI30新機能（偏差補正、ホバリング、ジンバル制御強化等）は維持
・写真撮影コマンドを takePhoto に変更
"""
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
    ...
    for ag in tree.findall(".//wpml:actionGroup", NS):
        for act in list(ag.findall("wpml:action", NS)):
            ...
            # ★変更点: takePhotoも削除対象に追加
            if f is not None and f.text in (
                "orientedShoot", "takePhoto", "startRecord", "stopRecord",
    ...
    # ★変更点: 写真撮影アクションを 'takePhoto' に変更
    # 6-1) 写真撮影（takePhotoコマンドに変更）
    if do_photo and not do_video:
        ...
        for i, pm in enumerate(pms):
            ...
            # ジンバル回転（オプション化） - これは takePhoto の前に行われるため、アングル指定に有効
            if do_gimbal:
                gimbal_action = etree.SubElement(grp, f"{{{NS['wpml']}}}action")
                etree.SubElement(gimbal_action, f"{{{NS['wpml']}}}actionId").text = str(nid)
                ...
                add("gimbalPitchRotateAngle", -90) # 真下向き
    ...
    if yaw_fix and yaw_angle is not None:
        ...
        # 8-2) 各ウェイポイントの設定を強制的に統一
        for pm in tree.findall(".//kml:Placemark", NS):
            # useGlobalHeadingParamを強制的に0に設定（個別設定優先）
            up = pm.find("wpml:useGlobalHeadingParam", NS)
            if up is None:
                etree.SubElement(pm, f"{{{NS['wpml']}}}useGlobalHeadingParam").text = "0"
            else:
                up.text = "0"
            
            # waypointHeadingParamが存在しない場合は新規作成
            ...
            if hp is None:
                ...
                etree.SubElement(hp, f"{{{NS['wpml']}}}waypointHeadingAngle").text = str(yaw_angle)
            else:
                # 既存の場合は値を更新
                m = hp.find("wpml:waypointHeadingMode", NS)
                a = hp.find("wpml:waypointHeadingAngle", NS)
        ...
        # 8-3) 安全策：rotateYawアクションを各ウェイポイントに追加
        pms = [
            p for p in tree.findall(".//kml:Placemark", NS)
            if p.find("wpml:index", NS) is not None
        ]
        ...
        for pm in pms:
            ...
            etree.SubElement(yaw_action, f"{{{NS['wpml']}}}actionId").text = str(next_id)
            ...
            etree.SubElement(yaw_param, f"{{{NS['wpml']}}}yawRotateAngle").text = str(yaw_angle)
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI30 - takePhoto版)")
