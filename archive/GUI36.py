...
# -*- coding: utf-8 -*-
"""
convert_height_gui_asl.py (ver. GUI34 - 理想形対応版)
変更点
・理想のKML形式に合わせて actionId を全て "0" に統一
・rotateYaw を最初に配置し、aircraftHeading と aircraftPathMode を使用
・重複する actionGroup 要素を削除
・アクション順序を理想形に合わせて修正
"""
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
    ...
    if do_photo and not do_video:
        ...
        for pm in pms:
            ...
            # 1) rotateYaw アクション（理想形に合わせて最初に配置）
            if yaw_fix and yaw_angle is not None:
            ...
            if do_gimbal:
                ...
                etree.SubElement(gimbal_action, f"{{{NS['wpml']}}}actionId").text = "0"
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI34 - 理想形対応版)")
