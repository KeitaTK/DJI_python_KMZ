...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
    ...
    if do_photo and not do_video:
        ...
        for i, pm in enumerate(pms):
            ...
            if yaw_fix and yaw_angle is not None:
                rotate_action = etree.SubElement(grp, f"{{{NS['wpml']}}}action")
                etree.SubElement(rotate_action, f"{{{NS['wpml']}}}actionId").text = str(nid)
                etree.SubElement(rotate_action, f"{{{NS['wpml']}}}actionActuatorFunc").text = "rotateYaw"
                yaw_param = etree.SubElement(rotate_action, f"{{{NS['wpml']}}}actionActuatorFuncParam")
            ...
            # hover（必要なら）
            if hover_time > 0:
                hover_action = etree.SubElement(grp, f"{{{NS['wpml']}}}action")
                etree.SubElement(hover_action, f"{{{NS['wpml']}}}actionId").text = str(nid)
    ...
    # 動画撮影（変更不要）
    # globalYaw設定、payload、速度、高度補正などは現状維持

    # 古いアクション（takePhoto含む）削除などは省略していますがそのままでOKです
