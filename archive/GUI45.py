...
"""
convert_height_gui_asl.py (ver. GUI36 – imageFormat 写真撮影対応版)

変更点
・写真撮影時の takePhoto アクションを削除し、
  imageFormat ベースの方式に変更
・各 Waypoint のアクション順序を GUI42 ライクに（Yaw→Gimbal→Hover）
・その他 GUI34 の機能はすべて維持
・Combobox イベントバインドを修正
"""
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
                do_gimbal, yaw_fix, yaw_angle, speed,
    ...
    if do_photo and sensor_modes:
        ...
        img_node = pp.find("wpml:imageFormat", NS)
        if img_node is None:
            img_node = etree.SubElement(pp, f"{{{NS['wpml']}}}imageFormat")
        img_node.text = fmt
    ...
    if do_photo and not do_video:
        pms = [p for p in tree.findall(".//kml:Placemark", NS)
        ...
        for pm in pms:
            ...
            # 2) gimbalRotate
            if do_gimbal:
                g_action = etree.SubElement(ag, f"{{{NS['wpml']}}}action")
                etree.SubElement(g_action, f"{{{NS['wpml']}}}actionId").text = "0"
                etree.SubElement(g_action, f"{{{NS['wpml']}}}actionActuatorFunc").text = "gimbalRotate"
                param = etree.SubElement(g_action, f"{{{NS['wpml']}}}actionActuatorFuncParam")
                ...
                etree.SubElement(param, f"{{{NS['wpml']}}}gimbalPitchRotateAngle").text = "-90"
            ...
            # 3) hover
            if hover_time > 0:
                h_action = etree.SubElement(ag, f"{{{NS['wpml']}}}action")
                etree.SubElement(h_action, f"{{{NS['wpml']}}}actionId").text = "0"
                etree.SubElement(h_action, f"{{{NS['wpml']}}}actionActuatorFunc").text = "hover"
                hover_param = etree.SubElement(h_action, f"{{{NS['wpml']}}}actionActuatorFuncParam")
                etree.SubElement(hover_param, f"{{{NS['wpml']}}}hoverTime").text = str(int(hover_time))

    # 8) 動画撮影ロジックは GUI34 のまま維持
    if do_video:
        pms = sorted(
