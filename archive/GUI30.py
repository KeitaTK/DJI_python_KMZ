...
"""
convert_height_gui_asl.py (ver. GUI30)

変更点
・カメラ撮影時のヨー角維持機能を強化
・動画撮影実装の最適化
・処理順序の改善によりヨー固定設定の確実性を向上
・各ウェイポイントでもヨー角設定を適用する機能を追加
・元機能の完全維持
"""
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
    ...
    if yaw_fix and yaw_angle is not None:
        ...
        for pm in tree.findall(".//kml:Placemark", NS):
            # 既存のヨー設定があるかチェック
            wp_heading = pm.find("wpml:waypointHeadingParam", NS)
            if wp_heading is None:
                # ヨー設定が存在しない場合は新規作成
                wp_heading = etree.SubElement(pm, f"{{{NS['wpml']}}}waypointHeadingParam")
                etree.SubElement(wp_heading, f"{{{NS['wpml']}}}waypointHeadingMode").text = "fixed"
                etree.SubElement(wp_heading, f"{{{NS['wpml']}}}waypointHeadingAngle").text = str(yaw_angle)
            else:
                # 既存のヨー設定を更新
                m = wp_heading.find("wpml:waypointHeadingMode", NS)
                a = wp_heading.find("wpml:waypointHeadingAngle", NS)
                if m is not None: 
                    m.text = "fixed"
                else:
                    etree.SubElement(wp_heading, f"{{{NS['wpml']}}}waypointHeadingMode").text = "fixed"
            ...
            # グローバル設定は使わない（個別設定を優先）
            use_global = pm.find("wpml:useGlobalHeadingParam", NS)
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI30)")
