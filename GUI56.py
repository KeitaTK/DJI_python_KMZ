...
"""
convert_height_gui_asl.py (ver. GUI58)

修正点
• 最後のウェイポイントでも機体の向き・ジンバル制御（他ウェイポイント同様）後に録画停止
• 動画モード時のホバリングを全ウェイポイントで、かつ機体向き・ジンバル制御後に実行
"""
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
        ...
        for pm in tree.findall(".//kml:Placemark", NS):
            ...
            if ce is not None and ce.text:
                ...
                try:
                    ...
                    pass
    for pm in tree.findall(".//wpml:ellipsoidHeight", NS) + tree.findall(".//wpml:height", NS):
        try:
            h = float(pm.text) + offset + (deviation[2] if deviation else 0)
            pm.text = str(h)
        except:
            pass
    ...
    if gh is not None:
        try:
            h = float(gh.text) + offset + (deviation[2] if deviation else 0)
            gh.text = str(h)
        except:
            pass
    ...
    # 速度設定
    for path in [(".//wpml:missionConfig", "globalTransitionalSpeed"),
                 (".//kml:Folder", "autoFlightSpeed")]:
        el = tree.find(f"{path[0]}/wpml:{path[1]}", NS)
        if el is not None:
            el.text = str(speed)
        else:
            parent = tree.find(path[0], NS)
            if parent is not None:
                etree.SubElement(parent, f"{{{NS['wpml']}}}{path[1]}").text = str(speed)
    for pm in tree.findall(".//kml:Placemark", NS):
        for tag in ("waypointSpeed", "useGlobalSpeed"):
            el = pm.find(f"wpml:{tag}", NS)
            if el is not None:
                el.text = str(speed if tag=="waypointSpeed" else 0)
            else:
                etree.SubElement(pm, f"{{{NS['wpml']}}}{tag}").text = str(speed if tag=="waypointSpeed" else 0)
    ...
    for i, pm in enumerate(pms):
        ...
        if do_photo and not do_video:
            ph_act = etree.SubElement(ag, f"{{{NS['wpml']}}}action")
            etree.SubElement(ph_act, f"{{{NS['wpml']}}}actionId").text = "0"
            etree.SubElement(ph_act, f"{{{NS['wpml']}}}actionActuatorFunc").text = "takePhoto"
            pp = etree.SubElement(ph_act, f"{{{NS['wpml']}}}actionActuatorFuncParam")
            etree.SubElement(pp, f"{{{NS['wpml']}}}fileSuffix").text = f"ウェイポイント{idx}"
            etree.SubElement(pp, f"{{{NS['wpml']}}}payloadPositionIndex").text = "0"
            etree.SubElement(pp, f"{{{NS['wpml']}}}useGlobalPayloadLensIndex").text = "1"
...
def process_kmz(path, offset, do_photo, do_video, video_suffix,
    ...
    try:
        ...
        log.insert(tk.END, f"使用センサー: {', '.join(sensor_modes) or 'デフォルト'}\n")
        if do_zoom and "Zoom" in sensor_modes:
        ...
        for kml in kmls:
            log.insert(tk.END, f"-- テンプレート: {os.path.basename(kml)} 読み込み\n")
            tree = etree.parse(kml, etree.XMLParser(remove_blank_text=True))
            global_h = tree.find(".//wpml:globalHeight", NS)
            gh = global_h.text if global_h is not None else "未設定"
            hm = tree.find(".//wpml:waylineCoordinateSysParam/wpml:heightMode", NS)
            hm_txt = hm.text if hm is not None else "未設定"
            log.insert(tk.END, f"グローバル高度: {gh}, 高度モード: {hm_txt}\n")

            wpms = [p for p in tree.findall(".//kml:Placemark", NS) if p.find("wpml:index", NS) is not None]
            log.insert(tk.END, f"総ウェイポイント数: {len(wpms)}\n")

            original_angles = extract_original_gimbal_angles(tree)
            log.insert(tk.END, f"元データジンバル情報: {len(original_angles)}箇所\n" if original_angles else "元データにジンバル情報なし\n")

            for pm in wpms:
                idx = int(pm.find("wpml:index", NS).text)
                hmd = pm.find("wpml:heightMode", NS)
                hm_t = hmd.text if hmd is not None else hm_txt
                he = pm.find("wpml:ellipsoidHeight", NS) or pm.find("wpml:height", NS)
                hv = he.text if he is not None else gh
                info = original_angles.get(idx, {})
                log.insert(tk.END,
                    f"[WP{idx}] モード={hm_t}, 高度={hv}, "
                    f"ピッチ={info.get('pitch','N/A')}°, ヨー={info.get('yaw','N/A')}°, "
                    f"機体Heading={info.get('heading','N/A')}°, "
                    f"ズーム={info.get('zoom_ratio','N/A')}倍\n"
                )
            ...
            log.insert(tk.END, f"書き出し: {out_path}\n")

        for name in ["res"]:
            srcs = glob.glob(os.path.join(wd, "**", name), recursive=True)
            if srcs:
                dst = os.path.join(outdir, os.path.basename(srcs[0]))
                if os.path.isdir(srcs[0]):
                    shutil.copytree(srcs[0], dst, dirs_exist_ok=True)
                else:
...
class AppGUI(ttk.Frame):
    ...
    def copy_reference_data(self):
        try:
            ref = REFERENCE_POINTS[self.ref_point_var.get()]
            now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            text = f"{now},{ref[0]},{ref[1]},{ref[2]}"
            pyperclip.copy(text)
    ...
    def get_params(self):
        ...
        if self.yf.get():
            ...
            if yval == "original":
                ...
                yaw_mode = "fixed"
                yaw_angle = float(yval) if isinstance(yval, (int,float)) else None
            if yaw_mode == "fixed" and self.yc.get() == "手動入力":
                try:
        ...
        if do_gimbal:
            gpv = GIMBAL_PITCH_OPTIONS.get(self.gp_mode.get())
            if gpv == "original":
                ...
                gimbal_pitch_mode = "fixed"
                gimbal_pitch_angle = float(gpv) if isinstance(gpv, (int,float)) else None
            if gimbal_pitch_mode == "fixed" and self.gp_mode.get() == "手動入力":
                try:
        ...
        zoom_mode = "none"
        if do_zoom:
            zmv = ZOOM_RATIO_OPTIONS.get(self.zm_mode.get())
            if zmv == "original":
                zoom_mode = "original"
            elif zmv == "custom":
                zoom_mode = "fixed"
                ...
                zoom_mode = "fixed"
                zoom_ratio = float(zmv) if isinstance(zmv, (int,float)) else None
            if zoom_mode == "fixed" and self.zm_mode.get() == "手動入力":
                try:
        ...
        if self.dc.get():
            try:
                ...
                safe, msg = check_deviation_safety(dev)
                if not safe:
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI58)")
