...
"""
convert_height_gui_asl.py (ver. GUI55 – ジンバル情報完全取得版)

修正点
• ジンバルピッチ、ヨー、機体ヘディングを完全取得
• ログに詳細なジンバル情報を出力
• 元の角度維持機能を正しく実装
"""
...
def extract_original_gimbal_angles(tree):
    """orientedShootアクションからジンバル情報を完全取得"""
    original = {}
...
def process_kmz(path, offset, do_photo, do_video, video_suffix,
    ...
    try:
        ...
        for kml in kmls:
            ...
            if original_angles:
                log.insert(tk.END, f"ジンバル情報を持つウェイポイント: {len(original_angles)}個\n")
            else:
                log.insert(tk.END, "元データにジンバル情報なし\n")
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI55)")
