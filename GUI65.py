...
def extract_original_gimbal_angles(tree):
    ...
    - まず従来の orientedShoot アクションを探し、
    - 見つからなければ gimbalRotate アクションから pitch/yaw を拾う。
    """
    original = {}
    ...
    for pm in tree.findall(".//kml:Placemark", NS):
        ...
        # 1) 従来ロジック：orientedShoot
        for action in pm.findall(".//wpml:action", NS):
            func = action.findtext("wpml:actionActuatorFunc", namespaces=NS)
            if func == "orientedShoot":
                param = action.find("wpml:actionActuatorFuncParam", NS)
                if param is None:
                    continue
                # pitch, yaw, heading, focalLength
                p = param.findtext("wpml:gimbalPitchRotateAngle", namespaces=NS)
                y = param.findtext("wpml:gimbalYawRotateAngle", namespaces=NS)
                h = param.findtext("wpml:aircraftHeading", namespaces=NS)
                f = param.findtext("wpml:focalLength", namespaces=NS)
                if p is not None: info["pitch"] = float(p)
                if y is not None: info["yaw"]   = float(y)
                if h is not None: info["heading"] = float(h)
                if f is not None:
                    fl = float(f)
                    info["focal_length"] = fl
                    info["zoom_ratio"] = fl / 24.0
                break  # orientedShoot があればここまで

        # 2) フォールバック：gimbalRotate から pitch/yaw
        if "pitch" not in info or "yaw" not in info:
            for action in pm.findall(".//wpml:action", NS):
                ...
                if func == "gimbalRotate":
                    ...
                    if param is None:
                        continue
                    # pitchRotateEnable=1 の時だけ pitch を取得
                    enable_p = param.findtext("wpml:gimbalPitchRotateEnable", namespaces=NS)
                    if enable_p == "1" and "pitch" not in info:
                        angle_p = param.findtext("wpml:gimbalPitchRotateAngle", namespaces=NS)
                        if angle_p is not None:
                            info["pitch"] = float(angle_p)
                    # yawRotateEnable=1 の時だけ yaw を取得
                    enable_y = param.findtext("wpml:gimbalYawRotateEnable", namespaces=NS)
                    if enable_y == "1" and "yaw" not in info:
                        angle_y = param.findtext("wpml:gimbalYawRotateAngle", namespaces=NS)
                        if angle_y is not None:
                            info["yaw"] = float(angle_y)
                    # 取得済み両方あれば抜け
                    if "pitch" in info and "yaw" in info:
                        break

        # 3) 機体ヘディング（rotateYaw アクション）取得
        for action in pm.findall(".//wpml:action", NS):
            func = action.findtext("wpml:actionActuatorFunc", namespaces=NS)
            if func == "rotateYaw":
                param = action.find("wpml:actionActuatorFuncParam", NS)
...
def process_kmz(path,
    ...
    try:
        ...
        for kml in kmls:
            ...
            log.insert(tk.END, f"元のヘディング設定取得完了\n")
