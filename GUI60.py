...
• 不足していたGUIメソッドを追加
• ファイル名形式を「元のファイル名_撮影モード_センサー頭文字」に変更
• ASLかどうかの判定を追加
"""
...
def prepare_output_dirs(input_kmz, offset, do_photo, do_video, sensor_modes):
    ...
    if sensor_modes:
        ...
        for sensor in sensor_modes:
            if sensor == "Wide":
                sensor_chars.append("W")
            elif sensor == "Zoom":
                sensor_chars.append("Z")
            elif sensor == "IR":
                sensor_chars.append("IR")
        sensor_suffix = "".join(sensor_chars)
    
    sign = "+" if offset >= 0 else "-"
    root = os.path.dirname(input_kmz)
    
    # 新しいファイル名形式: 元の名前_撮影モード_センサー頭文字_asl±オフセット
    if sensor_suffix:
        out_root = os.path.join(root, f"{base}_{mode_suffix}_{sensor_suffix}_asl{sign}{abs(offset)}")
    else:
        out_root = os.path.join(root, f"{base}_{mode_suffix}_asl{sign}{abs(offset)}")
...
def repackage_to_kmz(out_root, input_kmz, do_photo, do_video, sensor_modes):
    ...
    # 新しいKMZファイル名
    if sensor_suffix:
        out_kmz = os.path.join(os.path.dirname(out_root), f"{base}_{mode_suffix}_{sensor_suffix}.kmz")
    else:
        out_kmz = os.path.join(os.path.dirname(out_root), f"{base}_{mode_suffix}.kmz")
...
def check_deviation_safety(dev):
    if abs(dev[0]) > DEVIATION_THRESHOLD["lng"] or \
       ...
       abs(dev[2]) > DEVIATION_THRESHOLD["alt"]:
        return False, f"偏差が20mを超えています: 経度偏差{dev[0]:.8f}°, 緯度偏差{dev[1]:.8f}°, 標高偏差{dev[2]:.2f}m"
    return True, None
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
    ...
    log.insert(tk.END, f"高度オフセット: {offset}m\n")
    log.see(tk.END)
    ...
    for pm in tree.findall(".//kml:Placemark", NS):
        ...
        # ASL判定（EGM96、ASL、absoluteHeight等はASLとして扱う）
        ...
        for tag in ("height", "ellipsoidHeight"):
            ...
            if el is not None and el.text:
                try:
                    ...
                    if is_asl:
                        # ASLの場合はオフセット加算をスキップ
                        log.insert(tk.END, f"[WP {idx}] 高度モード: {height_mode} (ASL) → オフセット加算スキップ (元高度: {original_height}m)\n")
                        skipped_count += 1
                        # 偏差補正のみ適用
                        if deviation:
                            new_height = original_height + deviation[2]
                            el.text = str(new_height)
                            log.insert(tk.END, f"  偏差補正のみ適用: {original_height}m → {new_height}m\n")
                    else:
                        # ATLの場合は通常の処理
                        current_height = original_height + offset
                        if deviation:
                            current_height += deviation[2]
                        el.text = str(current_height)
                        log.insert(tk.END, f"[WP {idx}] 高度モード: {height_mode} (ATL) → オフセット加算実行: {original_height}m → {current_height}m\n")
                        processed_count += 1
                        
                except ValueError:
                    log.insert(tk.END, f"[WP {idx}] 高度値の解析エラー: {el.text}\n")
                    pass
    ...
    if gh is not None and gh.text:
        try:
            original_global_height = float(gh.text)
            is_global_asl = global_height_mode in ["ASL", "EGM96", "absoluteHeight", "WGS84"]
            
            if is_global_asl:
                log.insert(tk.END, f"グローバル高度: {global_height_mode} (ASL) → オフセット加算スキップ (元高度: {original_global_height}m)\n")
                # 偏差補正のみ適用
                if deviation:
                    new_global_height = original_global_height + deviation[2]
                    gh.text = str(new_global_height)
                    log.insert(tk.END, f"  偏差補正のみ適用: {original_global_height}m → {new_global_height}m\n")
            else:
                current_global_height = original_global_height + offset
                if deviation:
                    current_global_height += deviation[2]
                gh.text = str(current_global_height)
                log.insert(tk.END, f"グローバル高度: {global_height_mode} (ATL) → オフセット加算実行: {original_global_height}m → {current_global_height}m\n")
        except ValueError:
            log.insert(tk.END, f"グローバル高度値の解析エラー: {gh.text}\n")
            pass

    log.insert(tk.END, f"処理サマリー: オフセット加算実行 {processed_count}点、スキップ {skipped_count}点\n")
    
    # 高度モードをEGM96に統一（ASLだったものはそのまま、ATLだったもののみEGM96に変更）
    for hm in tree.findall(".//wpml:heightMode", NS):
        original_mode = hm.text
        if original_mode not in ["ASL", "EGM96", "absoluteHeight", "WGS84"]:
            hm.text = "EGM96"
            log.insert(tk.END, f"高度モード変更: {original_mode} → EGM96\n")

    # グローバル高度モードも同様に処理
    if global_height_mode_elem is not None:
        original_global_mode = global_height_mode_elem.text
        if original_global_mode not in ["ASL", "EGM96", "absoluteHeight", "WGS84"]:
            global_height_mode_elem.text = "EGM96"
            log.insert(tk.END, f"グローバル高度モード変更: {original_global_mode} → EGM96\n")

    log.insert(tk.END, "=== 高度補正処理完了 ===\n\n")
    log.see(tk.END)
...
def process_kmz(path, offset, do_photo, do_video, video_suffix,
    ...
    try:
        ...
        out_root, outdir = prepare_output_dirs(path, offset, do_photo, do_video, sensor_modes)
        ...
        for kml in kmls:
            ...
            convert_kml(tree, offset, do_photo, do_video, video_suffix,
                        ...
                        yaw_fix, yaw_angle, yaw_mode, speed, sensor_modes, hover_time,
                        do_zoom, zoom_ratio, zoom_mode,
                        coordinate_deviation, original_angles, log)

            # **追加: 高度補正処理完了ログ**
            log.insert(tk.END, f"高度補正処理完了\n\n")
            log.see(tk.END)
