...
"""
convert_height_gui_asl.py (ver. GUI27)

変更点
・動画撮影開始前の gimbalRotate を DJI ファーム準拠の詳細タグで生成
・GUIタイトルを ver. GUI27 に更新
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
    ...
    if yaw_fix and yaw_angle is not None:
        ...
        # 個別のウェイポイントヨー設定も更新または削除
        for pm in tree.findall(".//kml:Placemark", NS):
            wp_heading = pm.find("wpml:waypointHeadingParam", NS)
            if wp_heading is not None:
                # 既存の個別設定を削除し、グローバル設定を使用
                pm.remove(wp_heading)
                # useGlobalHeadingParamを設定
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI27)")
