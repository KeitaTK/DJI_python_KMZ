...
"""
convert_height_gui_asl.py (ver. GUI48 – ジンバルピッチ拡張版)

変更点
• 写真撮影・動画撮影時でもジンバル制御（ジンバルピッチ）の ON/OFF を選択可能にした  
• ジンバルピッチ ON 時は「真下（-90°）」「真ん前（0°）」「手動入力」の 3-択  
• ジンバルピッチ OFF 時はジンバル関連アクションを挿入しない  
• UI 表記「ジンバル制御」を「ジンバルピッチ」に変更
"""
...
def convert_kml(tree, offset, do_photo, do_video, video_suffix,
    ...
    # 0) ジンバルピッチ角度初期値
    if do_gimbal and gimbal_pitch_angle is None:
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI48 – ジンバルピッチ拡張版)")
