...
"""
convert_height_gui_asl.py (ver. GUI52 – 元角度維持完全版)

変更点
• ヨー固定・ジンバルピッチで「元の角度維持」選択時、orientedShootから抽出した角度を各ウェイポイントに適用
• 固定角度選択時のNoneType エラーを修正
• パラメータ取得・適用ロジックを完全に修正
"""
...
def extract_original_gimbal_angles(tree):
    """変換前KMLからorientedShootアクションの角度データを抽出"""
    original = {}
...
class AppGUI(ttk.Frame):
    ...
    def get_params(self):
        ...
        yaw_mode = "none"
        if self.yf.get():
            yaw_selection = self.yc.get()
            yval = YAW_OPTIONS.get(yaw_selection)
            if yval == "original":
                yaw_mode = "original"
            elif yval == "custom":
                yaw_mode = "fixed"
        ...
        if do_gimbal:
            gp_selection = self.gp_mode.get()
            gp_val = GIMBAL_PITCH_OPTIONS.get(gp_selection)
            if gp_val == "original":
...
def main():
    root = TkinterDnD.Tk()
    root.title("ATL→ASL 変換＋撮影制御ツール (ver. GUI52 – 元角度維持完全版)")
