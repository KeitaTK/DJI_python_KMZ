...
def extract_kmz(kmz_path, work_dir):
...
def repackage_to_kmz(base_out, input_kmz):
    inp_dir = os.path.dirname(input_kmz)
...
def convert_heights_and_mode(tree, offset,
    ...
    # 全ウェイポイント速度設定と useGlobalSpeed=0
    for pm in tree.findall(".//kml:Placemark", NS):
        ws = pm.find("wpml:waypointSpeed", NS)
        if ws is not None:
            ws.text = str(speed)
        else:
            new_ws = etree.SubElement(pm, "{http://www.dji.com/wpmz/1.0.6}waypointSpeed")
        ...
        if ugs is not None:
            ...
            new_ugs = etree.SubElement(pm, "{http://www.dji.com/wpmz/1.0.6}useGlobalSpeed")
            new_ugs.text = "0"

    # グローバル transitionalSpeed にも反映
    gts = tree.find(".//wpml:globalTransitionalSpeed", NS)
    if gts is not None:
        gts.text = str(speed)

    # アクション：orientedShoot/gimbalRotate 削除
    for ag in tree.findall(".//wpml:actionGroup", NS):
        for action in ag.findall("wpml:action", NS):
            ...
            if func == "orientedShoot" and not do_capture:
                ag.remove(action)
            if func == "gimbalRotate" and not do_capture and not do_gimbal:
                ag.remove(action)
...
def process_kmz(input_kmz, offset,
                do_capture, do_gimbal,
                fix_yaw, yaw_angle,
                speed,
                log):
...
class HeightSelector(ttk.Frame):
    def __init__(self, master):
        ...
        self.capture_chk = ttk.Checkbutton(self, text="写真撮影を行う", variable=self.capture_var, command=self.update_controls)
        self.capture_chk.grid(row=2, column=0, columnspan=3, sticky="w", pady=(5,0))

        # ジンバル
        self.gimbal_var = tk.BooleanVar(value=True)
        self.gimbal_chk = ttk.Checkbutton(self, text="ジンバル操作を保持", variable=self.gimbal_var)
        self.gimbal_chk.grid(row=3, column=0, columnspan=3, sticky="w", pady=(5,0))

        # ヨー固定
        self.yaw_fix_var = tk.BooleanVar(value=False)
        self.yaw_fix_chk = ttk.Checkbutton(self, text="ヨー角を固定", variable=self.yaw_fix_var, command=self.update_yaw_controls)
        self.yaw_fix_chk.grid(row=4, column=0, columnspan=3, sticky="w", pady=(5,0))

        self.yaw_combo = ttk.Combobox(self, values=list(YAW_FIX_OPTIONS.keys()), state="readonly", width=20)
    ...
    def on_select(self):
        c = self.combo.get()
        if HEIGHT_OPTIONS[c] == "custom":
            self.entry.config(state="normal"); self.entry.delete(0, tk.END); self.entry.focus()
        else:
            self.entry.config(state="disabled"); self.entry.delete(0, tk.END)

    def update_controls(self):
        if self.capture_var.get():
            self.gimbal_chk.state(["disabled"]); self.gimbal_var.set(True)
        else:
    ...
    def update_yaw_controls(self):
        if self.yaw_fix_var.get():
            ...
            self.yaw_combo.grid_forget(); self.yaw_entry.grid_forget()
        if self.yaw_combo.get() == "その他 – 手動入力":
            self.yaw_entry.config(state="normal"); self.yaw_entry.delete(0, tk.END); self.yaw_entry.focus()
        else:
            self.yaw_entry.config(state="disabled"); self.yaw_entry.delete(0, tk.END)

    def get_offset(self):
        c = self.combo.get(); v = HEIGHT_OPTIONS[c]
        if v == "custom":
            try: return float(self.entry.get())
            except: return 0.0
        return float(v)
    ...
    def get_capture_option(self):
        return self.capture_var.get()

    def get_gimbal_option(self):
        return self.gimbal_var.get()

    def get_yaw_fix_option(self):
        return self.yaw_fix_var.get()

    def get_yaw_angle(self):
        c = self.yaw_combo.get(); v = YAW_FIX_OPTIONS[c]
        if v == "custom":
            try: return float(self.yaw_entry.get())
            except: return 0.0
        return float(v)
...
def main():
    ...
    sel = HeightSelector(frame)
    sel.grid(row=0, column=0, columnspan=2, sticky="w", pady=(0,10))
    ...
    def on_drop(event):
        ...
        if not path.lower().endswith(".kmz"):
            messagebox.showwarning("警告", "KMZファイルのみ対応"); return
        offset = sel.get_offset()
        speed = sel.get_speed()
        do_capture = sel.get_capture_option()
        do_gimbal = sel.get_gimbal_option()
        fix_yaw = sel.get_yaw_fix_option()
        yaw_angle = sel.get_yaw_angle()
    ...
    frame.rowconfigure(2, weight=1); frame.columnconfigure(1, weight=1)
