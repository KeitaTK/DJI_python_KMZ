...
# -*- coding: utf-8 -*-
"""
convert_height_gui_asl_no_offset.py (ver. GUI62)
...
• 飛行経路に従う時、次のWPへの緯度・経度差が5m以下なら前WPの機体ヨー角を維持
"""
...
# 5mを緯度経度差に変換する定数 (緯度1度あたり約111km)
DIST_THRESHOLD_DEG = 5.0 / 111000.0
...
# --- 距離計算 ---------------------------------------------------------------
def waypoint_distance_deg(pm1, pm2):
    coord1 = pm1.find("kml:Point/kml:coordinates", NS).text.split(",")
    coord2 = pm2.find("kml:Point/kml:coordinates", NS).text.split(",")
    ...
    if heading_mode == "follow_wayline":
        ...
        for pm in waypoints:
            ...
            if hp is not None:
                pm.remove(hp)
        # 追加: 前WPのヨーを維持する判定
        for i, pm in enumerate(waypoints[1:], start=1):
            prev_pm = waypoints[i-1]
            dist = waypoint_distance_deg(prev_pm, pm)
            if dist <= DIST_THRESHOLD_DEG:
                prev_idx = int(prev_pm.find("wpml:index", NS).text)
                prev_heading = None
                if prev_idx in original_angles:
                    prev_heading = original_angles[prev_idx].get("heading")
                if prev_heading is not None:
                    ...
                    etree.SubElement(hp, f"{{{NS['wpml']}}}waypointHeadingAngle").text = str(int(prev_heading))
                    ...
                    log.insert(tk.END, f"[WP {int(pm.find('wpml:index',NS).text)}] 前WPヨー維持\n")
...
def convert_kml(tree,
    ...
    for i, pm in enumerate(pms):
        ...
        # tree.getpath()は使わず、直接パスを指定
        # この部分は実際には process_kmz 関数内で処理されるため削除
        log.insert(tk.END, "\n=== 処理完了 ===\n\n")
        log.see(tk.END)
    ...
    try:
        ...
        for kml in kmls:
            ...
            convert_kml(tree,
                        ...
                        heading_mode, original_heading_settings, log)
...
class AppGUI(ttk.Frame):
    def __init__(self, master):
        ...
        self.heading_mode_var = tk.StringVar(value="飛行経路に従う")
